...
export default class Generate extends Command {
  ...
  static args = {
    template: Args.string({
      required: true,
      description: 'The template to use, either a folder relative to the current or a folder included in the generator',
    }),
    output: Args.string({
      required: true,
  ...
  async run() {
    ...
    const outputBase = this.getFullPath(args.output, true)
    ...
    outputTemplate?.files.forEach(t => {
      if (t.usage === 'index') {
        ux.action.start(`Creating '${t.outputFile}' from template ${t.file}`)
        ...
        ux.action.start(`Creating services '${t.outputFile}' from template '${t.file}'`)
  ...
  private postProcess(input: string): string {
    // Do some post-processing on the template. HandlebarsJS doesn't support outputting '{' and '}'
  ...
  private getDeviceDescription(location: string, dataTypes: {[key: string]: string}, serviceData?: {[key: string]: any}): ExtendedSonosDescription | undefined | never {
    const intermediateFile = this.getFullPath(location)
    ...
    intermediate.services.forEach(service => {
      ...
      if (service.stateVariables !== undefined) {
      ...
      if (service.stateVariables !== undefined && service.actions !== undefined) {
