...
import { SonosServiceDocumentationAction, SonosServicesDocumentation } from '../models/sonos-service-documentation'
...
export default class Combine extends Command {
  ...
  async run() {
    ...
    let documentation: SonosServicesDocumentation | undefined = undefined;

    if (flags.docsFile) {
      let filename = this.getFullPathAndCreateDirectory(flags.docsFile);
    ...
    let combinedServices = devices.slice(0,1)[0];
